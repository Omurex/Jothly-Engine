...
//    rlb_Vector2 positions[PHYSAC_MAX_VERTICES];     // Polygon vertex positions vectors
//    rlb_Vector2 normals[PHYSAC_MAX_VERTICES];       // Polygon vertex normals vectors
//} PolygonData;
//
...
//    rlb_Vector2 position;                           // Physics body shape pivot
//    rlb_Vector2 velocity;                           // Current linear velocity applied to position
//    rlb_Vector2 force;                              // Current linear force (reset to 0 every step)
...
//    rlb_Vector2 normal;                             // Normal direction vector from 'a' to 'b'
//    rlb_Vector2 contacts[2];                        // Points of contact during collision
...
//PHYSACDEF PhysicsBody CreatePhysicsBodyCircle(rlb_Vector2 pos, float radius, float density);                    // Creates a new circle physics body with generic parameters
//PHYSACDEF PhysicsBody CreatePhysicsBodyRectangle(rlb_Vector2 pos, float width, float height, float density);    // Creates a new rectangle physics body with generic parameters
//PHYSACDEF PhysicsBody CreatePhysicsBodyPolygon(rlb_Vector2 pos, float radius, int sides, float density);        // Creates a new polygon physics body with generic parameters
//PHYSACDEF void PhysicsAddForce(PhysicsBody body, rlb_Vector2 force);                                            // Adds a force to a physics body
//PHYSACDEF void PhysicsAddTorque(PhysicsBody body, float amount);                                            // Adds an angular force to a physics body
//PHYSACDEF void PhysicsShatter(PhysicsBody body, rlb_Vector2 position, float force);                             // Shatters a polygon shape physics body to little physics bodies with explosion force
...
//PHYSACDEF rlb_Vector2 GetPhysicsShapeVertex(PhysicsBody body, int vertex);                                      // Returns transformed position of a body shape (body position + vertex transformed position)
...
//
//#if !defined(PHYSAC_STANDALONE)
//    #include "raymath.h"            // Required for: rlb_Vector2Add(), rlb_Vector2Subtract()
//#endif
//
...
//#define     PHYSAC_VECTOR_ZERO          (rlb_Vector2){ 0.0f, 0.0f }
//
////----------------------------------------------------------------------------------
...
//static rlb_Vector2 gravityForce = { 0.0f, 9.81f };              // Physics world gravity force
...
//static PolygonData CreateRectanglePolygon(rlb_Vector2 pos, rlb_Vector2 size);                                       // Creates a rectangle polygon shape based on a min and max positions
...
//static void FindIncidentFace(rlb_Vector2 *v0, rlb_Vector2 *v1, PhysicsShape ref, PhysicsShape inc, int index);      // Finds two polygon shapes incident face
//static int Clip(rlb_Vector2 normal, float clip, rlb_Vector2 *faceA, rlb_Vector2 *faceB);                                // Calculates clipping based on a normal and two faces
//static bool BiasGreaterThan(float valueA, float valueB);                                                    // Check if values are between bias range
//static rlb_Vector2 TriangleBarycenter(rlb_Vector2 v1, rlb_Vector2 v2, rlb_Vector2 v3);                                      // Returns the barycenter of a triangle given by 3 points
//
...
//
//// Math functions
//static rlb_Vector2 MathCross(float value, rlb_Vector2 vector);                                                      // Returns the cross product of a vector and a value
//static float MathCrossrlb_Vector2(rlb_Vector2 v1, rlb_Vector2 v2);                                                      // Returns the cross product of two vectors
//static float MathLenSqr(rlb_Vector2 vector);                                                                    // Returns the len square root of a vector
//static float MathDot(rlb_Vector2 v1, rlb_Vector2 v2);                                                               // Returns the dot product of two vectors
//static inline float DistSqr(rlb_Vector2 v1, rlb_Vector2 v2);                                                        // Returns the square root of distance between two vectors
//static void MathNormalize(rlb_Vector2 *vector);                                                                 // Returns the normalized values of a vector
//#if defined(PHYSAC_STANDALONE)
//static rlb_Vector2 rlb_Vector2Add(rlb_Vector2 v1, rlb_Vector2 v2);                                                          // Returns the sum of two given vectors
//static rlb_Vector2 rlb_Vector2Subtract(rlb_Vector2 v1, rlb_Vector2 v2);                                                     // Returns the subtract of two given vectors
//#endif
//
...
//static inline rlb_Vector2 Mat2Multiplyrlb_Vector2(Mat2 matrix, rlb_Vector2 vector);                                     // Multiplies a vector by a matrix 2x2
//
////----------------------------------------------------------------------------------
...
//PHYSACDEF PhysicsBody CreatePhysicsBodyCircle(rlb_Vector2 pos, float radius, float density)
//{
...
//PHYSACDEF PhysicsBody CreatePhysicsBodyRectangle(rlb_Vector2 pos, float width, float height, float density)
//{
//    PhysicsBody newBody = (PhysicsBody)PHYSAC_MALLOC(sizeof(PhysicsBodyData));
//    usedMemory += sizeof(PhysicsBodyData);
//
...
//        newBody->enabled = true;
//        newBody->position = pos;
//        newBody->velocity = (rlb_Vector2){ 0.0f };
//        newBody->force = (rlb_Vector2){ 0.0f };
//        newBody->angularVelocity = 0.0f;
//        newBody->torque = 0.0f;
//        newBody->orient = 0.0f;
...
//        newBody->shape.transform = Mat2Radians(0.0f);
//        newBody->shape.vertexData = CreateRectanglePolygon(pos, (rlb_Vector2){ width, height });
//
//        // Calculate centroid and moment of inertia
//        rlb_Vector2 center = { 0.0f, 0.0f };
//        float area = 0.0f;
//        float inertia = 0.0f;
//
...
//            rlb_Vector2 p1 = newBody->shape.vertexData.positions[i];
//            int nextIndex = (((i + 1) < newBody->shape.vertexData.vertexCount) ? (i + 1) : 0);
//            rlb_Vector2 p2 = newBody->shape.vertexData.positions[nextIndex];
//
//            float D = MathCrossrlb_Vector2(p1, p2);
...
//PHYSACDEF PhysicsBody CreatePhysicsBodyPolygon(rlb_Vector2 pos, float radius, int sides, float density)
//{
//    PhysicsBody newBody = (PhysicsBody)PHYSAC_MALLOC(sizeof(PhysicsBodyData));
//    usedMemory += sizeof(PhysicsBodyData);
//
...
//        {
//            // Triangle vertices, third vertex implied as (0, 0)
//            rlb_Vector2 position1 = newBody->shape.vertexData.positions[i];
//            int nextIndex = (((i + 1) < newBody->shape.vertexData.vertexCount) ? (i + 1) : 0);
//            rlb_Vector2 position2 = newBody->shape.vertexData.positions[nextIndex];
//
//            float cross = MathCrossrlb_Vector2(position1, position2);
...
//PHYSACDEF void PhysicsAddForce(PhysicsBody body, rlb_Vector2 force)
//{
//    if (body != NULL)
...
//PHYSACDEF void PhysicsShatter(PhysicsBody body, rlb_Vector2 position, float force)
//{
//    if (body != NULL)
//    {
...
//            {
//                rlb_Vector2 positionA = body->position;
//                rlb_Vector2 positionB = Mat2Multiplyrlb_Vector2(body->shape.transform, Vector2Add(body->position, vertexData.positions[i]));
//                int nextIndex = (((i + 1) < vertexData.vertexCount) ? (i + 1) : 0);
//                rlb_Vector2 positionC = Mat2Multiplyrlb_Vector2(body->shape.transform, Vector2Add(body->position, vertexData.positions[nextIndex]));
//
...
//                rlb_Vector2 bodyPos = body->position;
//                rlb_Vector2 *vertices = (rlb_Vector2*)malloc(sizeof(rlb_Vector2) * count);
...
//                    rlb_Vector2 center = TriangleBarycenter(vertices[i], vertices[nextIndex], PHYSAC_VECTOR_ZERO);
//                    center = Vector2Add(bodyPos, center);
//                    rlb_Vector2 offset = Vector2Subtract(center, bodyPos);
//
...
//                        rlb_Vector2 face = Vector2Subtract(newData.positions[nextVertex], newData.positions[j]);
//
//                        newData.normals[j] = (rlb_Vector2){ face.y, -face.x };
...
//                    {
//                        // Triangle vertices, third vertex implied as (0, 0)
//                        rlb_Vector2 p1 = newBody->shape.vertexData.positions[j];
//                        int nextVertex = (((j + 1) < newBody->shape.vertexData.vertexCount) ? (j + 1) : 0);
//                        rlb_Vector2 p2 = newBody->shape.vertexData.positions[nextVertex];
//
//                        float D = MathCrossrlb_Vector2(p1, p2);
//                        float triangleArea = D/2;
//
...
//                    rlb_Vector2 pointA = newBody->position;
//                    rlb_Vector2 pointB = Vector2Subtract(newData.positions[1], newData.positions[0]);
//                    pointB.x /= 2.0f;
//                    pointB.y /= 2.0f;
//                    rlb_Vector2 forceDirection = Vector2Subtract(Vector2Add(pointA, Vector2Add(newData.positions[0], pointB)), newBody->position);
...
//PHYSACDEF rlb_Vector2 GetPhysicsShapeVertex(PhysicsBody body, int vertex)
//{
//    rlb_Vector2 position = { 0.0f, 0.0f };
//
//    if (body != NULL)
//    {
//        switch (body->shape.type)
...
//            {
//                PolygonData vertexData = body->shape.vertexData;
//                position = Vector2Add(body->position, Mat2Multiplyrlb_Vector2(body->shape.transform, vertexData.positions[vertex]));
//            } break;
//            default: break;
//        }
//    }
...
//        rlb_Vector2 face = Vector2Subtract(data.positions[nextIndex], data.positions[i]);
//
//        data.normals[i] = (rlb_Vector2){ face.y, -face.x };
...
//static PolygonData CreateRectanglePolygon(rlb_Vector2 pos, rlb_Vector2 size)
//{
//    PolygonData data = { 0 };
//    data.vertexCount = 4;
//
//    // Calculate polygon vertices positions
//    data.positions[0] = (rlb_Vector2){ pos.x + size.x/2, pos.y - size.y/2 };
//    data.positions[1] = (rlb_Vector2){ pos.x + size.x/2, pos.y + size.y/2 };
//    data.positions[2] = (rlb_Vector2){ pos.x - size.x/2, pos.y + size.y/2 };
//    data.positions[3] = (rlb_Vector2){ pos.x - size.x/2, pos.y - size.y/2 };
//
//    // Calculate polygon faces normals
//    for (int i = 0; i < data.vertexCount; i++)
//    {
...
//    rlb_Vector2 normal = Vector2Subtract(bodyB->position, bodyA->position);
//
...
//        manifold->normal = (rlb_Vector2){ 1.0f, 0.0f };
//        manifold->contacts[0] = bodyA->position;
//    }
//    else
//    {
//        manifold->penetration = radius - distance;
//        manifold->normal = (rlb_Vector2){ normal.x/distance, normal.y/distance }; // Faster than using MathNormalize() due to sqrt is already performed
//        manifold->contacts[0] = (rlb_Vector2){ manifold->normal.x*bodyA->shape.radius + bodyA->position.x, manifold->normal.y*bodyA->shape.radius + bodyA->position.y };
//    }
//
...
//    rlb_Vector2 center = bodyA->position;
//    center = Mat2Multiplyrlb_Vector2(Mat2Transpose(bodyB->shape.transform), Vector2Subtract(center, bodyB->position));
//
...
//    rlb_Vector2 v1 = vertexData.positions[faceNormal];
//    int nextIndex = (((faceNormal + 1) < vertexData.vertexCount) ? (faceNormal + 1) : 0);
//    rlb_Vector2 v2 = vertexData.positions[nextIndex];
//
//    // Check to see if center is within polygon
//    if (separation < PHYSAC_EPSILON)
//    {
//        manifold->contactsCount = 1;
//        rlb_Vector2 normal = Mat2Multiplyrlb_Vector2(bodyB->shape.transform, vertexData.normals[faceNormal]);
//        manifold->normal = (rlb_Vector2){ -normal.x, -normal.y };
//        manifold->contacts[0] = (rlb_Vector2){ manifold->normal.x*bodyA->shape.radius + bodyA->position.x, manifold->normal.y*bodyA->shape.radius + bodyA->position.y };
//        manifold->penetration = bodyA->shape.radius;
//        return;
//    }
...
//
//        manifold->contactsCount = 1;
//        rlb_Vector2 normal = Vector2Subtract(v1, center);
//        normal = Mat2Multiplyrlb_Vector2(bodyB->shape.transform, normal);
//        MathNormalize(&normal);
//        manifold->normal = normal;
//        v1 = Mat2Multiplyrlb_Vector2(bodyB->shape.transform, v1);
...
//
//        manifold->contactsCount = 1;
//        rlb_Vector2 normal = Vector2Subtract(v2, center);
//        v2 = Mat2Multiplyrlb_Vector2(bodyB->shape.transform, v2);
...
//    {
//        rlb_Vector2 normal = vertexData.normals[faceNormal];
//
...
//    rlb_Vector2 incidentFace[2];
//    FindIncidentFace(&incidentFace[0], &incidentFace[1], refPoly, incPoly, referenceIndex);
//
//    // Setup reference face vertices
//    PolygonData refData = refPoly.vertexData;
//    rlb_Vector2 v1 = refData.positions[referenceIndex];
//    referenceIndex = (((referenceIndex + 1) < refData.vertexCount) ? (referenceIndex + 1) : 0);
//    rlb_Vector2 v2 = refData.positions[referenceIndex];
//
//    // Transform vertices to world space
//    v1 = Mat2Multiplyrlb_Vector2(refPoly.transform, v1);
//    v1 = Vector2Add(v1, refPoly.body->position);
//    v2 = Mat2Multiplyrlb_Vector2(refPoly.transform, v2);
//    v2 = Vector2Add(v2, refPoly.body->position);
//
//    // Calculate reference face side normal in world space
//    rlb_Vector2 sidePlaneNormal = Vector2Subtract(v2, v1);
//    MathNormalize(&sidePlaneNormal);
//
//    // Orthogonalize
//    rlb_Vector2 refFaceNormal = { sidePlaneNormal.y, -sidePlaneNormal.x };
...
//    if (Clip((rlb_Vector2){ -sidePlaneNormal.x, -sidePlaneNormal.y }, negSide, &incidentFace[0], &incidentFace[1]) < 2)
//        return;
//
...
//    manifold->normal = (flip ? (rlb_Vector2){ -refFaceNormal.x, -refFaceNormal.y } : refFaceNormal);
//
...
//        rlb_Vector2 radiusA = Vector2Subtract(manifold->contacts[i], bodyA->position);
//        rlb_Vector2 radiusB = Vector2Subtract(manifold->contacts[i], bodyB->position);
//
//        rlb_Vector2 crossA = MathCross(bodyA->angularVelocity, radiusA);
//        rlb_Vector2 crossB = MathCross(bodyB->angularVelocity, radiusB);
//
//        rlb_Vector2 radiusV = { 0.0f, 0.0f };
...
//        if (MathLenSqr(radiusV) < (MathLenSqr((rlb_Vector2){ gravityForce.x*deltaTime/1000, gravityForce.y*deltaTime/1000 }) + PHYSAC_EPSILON))
...
//            return;
//
//        float raCrossN = MathCrossrlb_Vector2(radiusA, manifold->normal);
//        float rbCrossN = MathCrossrlb_Vector2(radiusB, manifold->normal);
//
...
//        rlb_Vector2 impulseV = { manifold->normal.x*impulse, manifold->normal.y*impulse };
//
//        if (bodyA->enabled)
//        {
...
//                bodyA->angularVelocity += bodyA->inverseInertia*MathCrossrlb_Vector2(radiusA, (rlb_Vector2){ -impulseV.x, -impulseV.y });
//        }
//
//        if (bodyB->enabled)
//        {
...
//                bodyB->angularVelocity += bodyB->inverseInertia*MathCrossrlb_Vector2(radiusB, impulseV);
//        }
//
...
//        radiusV.y = bodyB->velocity.y + MathCross(bodyB->angularVelocity, radiusB).y - bodyA->velocity.y - MathCross(bodyA->angularVelocity, radiusA).y;
//
//        rlb_Vector2 tangent = { radiusV.x - (manifold->normal.x*MathDot(radiusV, manifold->normal)), radiusV.y - (manifold->normal.y*MathDot(radiusV, manifold->normal)) };
...
//
//        // Apply coulumb's law
//        rlb_Vector2 tangentImpulse = { 0.0f, 0.0f };
//        if (absImpulseTangent < impulse*manifold->staticFriction)
//            tangentImpulse = (rlb_Vector2){ tangent.x*impulseTangent, tangent.y*impulseTangent };
//        else
//            tangentImpulse = (rlb_Vector2){ tangent.x*-impulse*manifold->dynamicFriction, tangent.y*-impulse*manifold->dynamicFriction };
//
...
//
//            if (!bodyA->freezeOrient)
//                bodyA->angularVelocity += bodyA->inverseInertia*MathCrossrlb_Vector2(radiusA, (rlb_Vector2){ -tangentImpulse.x, -tangentImpulse.y });
//        }
//
//        if (bodyB->enabled)
//        {
...
//
//            if (!bodyB->freezeOrient)
//                bodyB->angularVelocity += bodyB->inverseInertia*MathCrossrlb_Vector2(radiusB, tangentImpulse);
//        }
//    }
//}
//
...
//        return;
//
//    rlb_Vector2 correction = { 0.0f, 0.0f };
...
//static rlb_Vector2 GetSupport(PhysicsShape shape, rlb_Vector2 dir)
//{
//    float bestProjection = -PHYSAC_FLT_MAX;
//    rlb_Vector2 bestVertex = { 0.0f, 0.0f };
//    PolygonData data = shape.vertexData;
//
//    for (int i = 0; i < data.vertexCount; i++)
//    {
//        rlb_Vector2 vertex = data.positions[i];
...
//        rlb_Vector2 normal = dataA.normals[i];
//        rlb_Vector2 transNormal = Mat2Multiplyrlb_Vector2(shapeA.transform, normal);
//
//        // Transform face normal into B shape's model space
//        Mat2 buT = Mat2Transpose(shapeB.transform);
//        normal = Mat2Multiplyrlb_Vector2(buT, transNormal);
//
//        // Retrieve support point from B shape along -n
//        rlb_Vector2 support = GetSupport(shapeB, (rlb_Vector2){ -normal.x, -normal.y });
//
//        // Retrieve vertex on face from A shape, transform into B shape's model space
//        rlb_Vector2 vertex = dataA.positions[i];
//        vertex = Mat2Multiplyrlb_Vector2(shapeA.transform, vertex);
//        vertex = Vector2Add(vertex, shapeA.body->position);
//        vertex = Vector2Subtract(vertex, shapeB.body->position);
//        vertex = Mat2Multiplyrlb_Vector2(buT, vertex);
//
...
//static void FindIncidentFace(rlb_Vector2 *v0, rlb_Vector2 *v1, PhysicsShape ref, PhysicsShape inc, int index)
//{
//    PolygonData refData = ref.vertexData;
//    PolygonData incData = inc.vertexData;
//
//    rlb_Vector2 referenceNormal = refData.normals[index];
//
//    // Calculate normal in incident's frame of reference
//    referenceNormal = Mat2Multiplyrlb_Vector2(ref.transform, referenceNormal); // To world space
//    referenceNormal = Mat2Multiplyrlb_Vector2(Mat2Transpose(inc.transform), referenceNormal); // To incident's model space
//
...
//    *v0 = Mat2Multiplyrlb_Vector2(inc.transform, incData.positions[incidentFace]);
//    *v0 = Vector2Add(*v0, inc.body->position);
//    incidentFace = (((incidentFace + 1) < incData.vertexCount) ? (incidentFace + 1) : 0);
//    *v1 = Mat2Multiplyrlb_Vector2(inc.transform, incData.positions[incidentFace]);
//    *v1 = Vector2Add(*v1, inc.body->position);
//}
//
//// Calculates clipping based on a normal and two faces
//static int Clip(rlb_Vector2 normal, float clip, rlb_Vector2 *faceA, rlb_Vector2 *faceB)
//{
//    int sp = 0;
//    rlb_Vector2 out[2] = { *faceA, *faceB };
//
...
//        out[sp] = *faceA;
//        rlb_Vector2 delta = Vector2Subtract(*faceB, *faceA);
//        delta.x *= alpha;
//        delta.y *= alpha;
...
//static rlb_Vector2 TriangleBarycenter(rlb_Vector2 v1, rlb_Vector2 v2, rlb_Vector2 v3)
//{
//    rlb_Vector2 result = { 0.0f, 0.0f };
//
...
//static inline rlb_Vector2 MathCross(float value, rlb_Vector2 vector)
//{
//    return (rlb_Vector2){ -value*vector.y, value*vector.x };
//}
//
//// Returns the cross product of two vectors
//static inline float MathCrossrlb_Vector2(rlb_Vector2 v1, rlb_Vector2 v2)
//{
//    return (v1.x*v2.y - v1.y*v2.x);
//}
//
//// Returns the len square root of a vector
//static inline float MathLenSqr(rlb_Vector2 vector)
//{
//    return (vector.x*vector.x + vector.y*vector.y);
//}
//
//// Returns the dot product of two vectors
//static inline float MathDot(rlb_Vector2 v1, rlb_Vector2 v2)
//{
//    return (v1.x*v2.x + v1.y*v2.y);
//}
//
//// Returns the square root of distance between two vectors
//static inline float DistSqr(rlb_Vector2 v1, rlb_Vector2 v2)
//{
//    rlb_Vector2 dir = Vector2Subtract(v1, v2);
//    return MathDot(dir, dir);
//}
//
//// Returns the normalized values of a vector
//static void MathNormalize(rlb_Vector2 *vector)
//{
//    float length, ilength;
//
//    rlb_Vector2 aux = *vector;
...
//static inline rlb_Vector2 rlb_Vector2Add(rlb_Vector2 v1, rlb_Vector2 v2)
//{
//    return (rlb_Vector2){ v1.x + v2.x, v1.y + v2.y };
//}
//
//// Returns the subtract of two given vectors
//static inline rlb_Vector2 rlb_Vector2Subtract(rlb_Vector2 v1, rlb_Vector2 v2)
//{
//    return (rlb_Vector2){ v1.x - v2.x, v1.y - v2.y };
//}
//#endif
...
//static inline rlb_Vector2 Mat2Multiplyrlb_Vector2(Mat2 matrix, rlb_Vector2 vector)
//{
//    return (rlb_Vector2){ matrix.m00*vector.x + matrix.m01*vector.y, matrix.m10*vector.x + matrix.m11*vector.y };
//}
//
